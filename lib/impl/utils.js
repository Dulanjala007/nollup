...
async function combineSourceMapChainFast (mapChain, original_code, filepath) {
    let map;
    mapChain = mapChain.filter(o => o.map && o.map.mappings).reverse();
    ...
    if (mapChain.length > 1) {
        ...
        let mapGenerator = SourceMapFast.SourceMapGenerator.fromSourceMap(await new SourceMapFast.SourceMapConsumer(mapChain[0].map));
